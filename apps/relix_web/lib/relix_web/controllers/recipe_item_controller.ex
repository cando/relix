defmodule RelixWeb.RecipeItemController do
  ...
  def index(conn, %{"recipe_id" => recipe_id}) do
    with {:ok, recipe} <- recipe_list().get_recipe_by_id(recipe_id) do
      render(conn, "index.json", recipe_items: recipe.items)
    end
  end

  def create(conn, %{"recipe_id" => recipe_id, "id" => name, "value" => value}) do
    with {:ok, updated_recipe} <- recipe_list().add_or_update_item(recipe_id, name, value) do
      conn
      |> put_status(:created)
  ...
  def show(conn, %{"recipe_id" => recipe_id, "id" => name}) do
    with {:ok, recipe} <- recipe_list().get_recipe_by_id(recipe_id),
  ...
  def update(conn, %{"recipe_id" => recipe_id, "id" => name, "value" => value}) do
    with {:ok, _updated_recipe} <- recipe_list().add_or_update_item(recipe_id, name, value) do
      send_resp(conn, :no_content, "")
    end
  end

  def delete(conn, %{"recipe_id" => recipe_id, "id" => name}) do
    with {:ok, _updated_recipe} <- recipe_list().delete_item(recipe_id, name) do
      send_resp(conn, :no_content, "")
    end
  end
